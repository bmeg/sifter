...
func (tw *TableWriteStep) Init(task *manager.Task) {
	sep := '\t'
	if tw.Sep != "" {
		sep = rune(tw.Sep[0])
	}
...
func (tw *TableWriteStep) Run(i map[string]interface{}, task *manager.Task) map[string]interface{} {
...
func (tr *TableReplaceStep) Init(task *manager.Task) error {
	if tr.Input != "" {
...
func (tr *TableReplaceStep) Run(i map[string]interface{}, task *manager.Task) map[string]interface{} {
...
func (tr *TableLookupStep) Init(task *manager.Task) error {
	input, err := evaluate.ExpressionString(tr.Input, task.Inputs, nil)
	inputPath, err := task.Path(input)
	if err != nil {
		return err
...
func (tr *TableLookupStep) Run(i map[string]interface{}, task *manager.Task) map[string]interface{} {
